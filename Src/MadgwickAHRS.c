...
void MadgwickAHRSInit(quanternion *q, float sampleFreqMS){
	q->sampleFreq = 1/sampleFreqMS;		// sample frequency in Hz
	q->beta = -0.2;								// 2 * proportional gain (Kp)
	q->q0 = 1.0f;	// quaternion of sensor frame relative to auxiliary frame
	q->q1 = 0.0f;
	q->q2 = 0.0f;
	q->q3 = 0.0f;
}
...
void MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, quanternion *q) {
	...
	q->q0 += qDot1 * (1.0f / q->sampleFreq);
	q->q1 += qDot2 * (1.0f / q->sampleFreq);
	q->q2 += qDot3 * (1.0f / q->sampleFreq);
	q->q3 += qDot4 * (1.0f / q->sampleFreq);
